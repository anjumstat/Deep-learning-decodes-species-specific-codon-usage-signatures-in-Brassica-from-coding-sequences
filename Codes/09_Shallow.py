# -*- coding: utf-8 -*-
"""
Created on Tue Jun 24 16:44:46 2025
...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar  8 19:40:24 2025
...
# Define Shallow NN model with one hidden layer
...
best_val_acc = 0.0
best_model_path = os.path.join(output_dir, "Best_Shallow_NN_Model.h5")
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train)):
    ...
    # Train model
    model = build_shallow_nn(X_train.shape[1], num_classes)
    ...
    # Save model
    model_path = os.path.join(output_dir, f"Shallow_NN_fold{fold+1}.h5")
...
# Compute average metrics after cross-validation
avg_metrics = {key: np.mean(value) for key, value in metrics.items()}

# Save metrics as CSV (with average values of the 10 folds)
metrics_df = pd.DataFrame(avg_metrics, index=[0])
metrics_df.to_csv(os.path.join(output_dir, "Shallow_NN_Metrics_Avg.csv"), index=False)
...
plt.title('Confusion Matrix - Shallow NN', fontsize=16)
...
plt.plot(avg_train_acc, label='Training Accuracy', linewidth=4, marker='o', color='blue')
plt.plot(avg_val_acc, label='Validation Accuracy', linewidth=4, marker='s', color='green')
plt.xlabel('Epochs', fontsize=18)
plt.ylabel('Accuracy', fontsize=18)
plt.title('Training and Validation Accuracy - Shallow NN', fontsize=20)
# Change font size for x and y axis tick labels
plt.xticks(fontsize=16)  # Set the font size for x-axis ticks
plt.yticks(fontsize=16)  # Set the font size for y-axis ticks
