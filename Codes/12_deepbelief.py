# -*- coding: utf-8 -*-
"""
Created on Sat Mar  8 11:22:00 2025
...
# Define DBN-like model (Deep Belief Network Simulation with stacked fully connected layers)
def build_dbn(input_shape, num_classes):
...
best_val_acc = 0.0
best_model_path = os.path.join(output_dir, "Best_DBN_Model.h5")
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train)):
    ...
    # Train model
    model = build_dbn(X_train.shape[1], num_classes)
    ...
    # Save model
    model_path = os.path.join(output_dir, f"DBN_fold{fold+1}.h5")
...
metrics_df.to_csv(os.path.join(output_dir, "DBN_Metrics_Avg.csv"), index=False)
...
plt.title('Confusion Matrix - DBN', fontsize=16)
...
plt.title('Training and Validation Accuracy - DBN', fontsize=16)
plt.legend()
plt.grid(True)
